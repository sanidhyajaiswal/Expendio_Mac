...
struct ExpenseListView: View {
    ...
    private var filteredExpenses: [Expense] {
        expenses.filter { expense in
            let matchesSearch = searchText.isEmpty || expense.title.localizedCaseInsensitiveContains(searchText) || expense.notes.localizedCaseInsensitiveContains(searchText)
    ...
    private var headerBar: some View {
        HStack(spacing: 16) {
            ...
            Spacer()
            Button { showAddSheet = true } label: { HStack(spacing: 6) { Image(systemName: "plus"); Text("Add Expense") }.gradientButton() }.buttonStyle(.plain)
    ...
    private var filterBar: some View {
        HStack(spacing: 12) {
            HStack(spacing: 8) {
                ...
                TextField("Search expenses...", text: $searchText).textFieldStyle(.plain).font(.system(size: 13)).foregroundColor(AppTheme.textPrimary)
                if !searchText.isEmpty { Button { searchText = "" } label: { Image(systemName: "xmark.circle.fill").foregroundColor(AppTheme.textMuted) }.buttonStyle(.plain) }
            }.padding(.horizontal, 12).padding(.vertical, 8)
            .background(RoundedRectangle(cornerRadius: 10).fill(AppTheme.surfaceElevated).overlay(RoundedRectangle(cornerRadius: 10).stroke(AppTheme.border.opacity(0.5), lineWidth: 1))).frame(maxWidth: 300)
            
            Menu {
                Button("All Categories") { selectedCategory = nil }; Divider()
                ForEach(categories, id: \.id) { cat in Button { selectedCategory = cat } label: { Label(cat.name, systemImage: cat.icon) } }
            } label: {
                HStack(spacing: 6) { Image(systemName: "line.3.horizontal.decrease.circle"); Text(selectedCategory?.name ?? "All Categories").lineLimit(1); Image(systemName: "chevron.down").font(.system(size: 10)) }
                    .font(.system(size: 13, weight: .medium)).foregroundColor(AppTheme.textSecondary).padding(.horizontal, 12).padding(.vertical, 8)
    ...
    private var expenseTable: some View {
        VStack(spacing: 0) {
            HStack(spacing: 0) {
                Text("DATE").frame(width: 110, alignment: .leading); Text("TITLE").frame(maxWidth: .infinity, alignment: .leading)
                Text("CATEGORY").frame(width: 160, alignment: .leading); Text("AMOUNT").frame(width: 120, alignment: .trailing)
                Text("NOTES").frame(width: 150, alignment: .leading).padding(.leading, 16); Text("").frame(width: 80)
            }.font(.system(size: 11, weight: .semibold)).foregroundColor(AppTheme.textMuted).padding(.horizontal, 20).padding(.vertical, 10).background(AppTheme.surface.opacity(0.5))
            Divider().overlay(AppTheme.border.opacity(0.3))
            ScrollView {
                LazyVStack(spacing: 0) {
                    ForEach(filteredExpenses, id: \.id) { expense in
                        if editingExpenseId == expense.id { editingRow(expense) } else { displayRow(expense) }
                        Divider().overlay(AppTheme.border.opacity(0.15))
                    }
                }
            }
        }.glassCard(padding: 0).padding(.horizontal, 32).padding(.bottom, 32)
    }
    
    private func displayRow(_ expense: Expense) -> some View {
        HStack(spacing: 0) {
            Text(expense.date, format: .dateTime.month(.abbreviated).day().year()).font(.system(size: 13)).foregroundColor(AppTheme.textSecondary).frame(width: 110, alignment: .leading)
            HStack(spacing: 8) {
            ...
            HStack(spacing: 6) {
                if let cat = expense.category { Circle().fill(cat.color).frame(width: 8, height: 8); Text(cat.name).lineLimit(1) } else { Text("Uncategorized") }
            ...
            HStack(spacing: 8) {
                Button { startEditing(expense) } label: { Image(systemName: "pencil").font(.system(size: 12)).foregroundColor(AppTheme.textSecondary).frame(width: 28, height: 28).background(RoundedRectangle(cornerRadius: 6).fill(AppTheme.surfaceElevated)) }.buttonStyle(.plain).opacity(hoveredExpenseId == expense.id ? 1 : 0)
                Button { deleteExpense(expense) } label: { Image(systemName: "trash").font(.system(size: 12)).foregroundColor(AppTheme.danger).frame(width: 28, height: 28).background(RoundedRectangle(cornerRadius: 6).fill(AppTheme.danger.opacity(0.1))) }.buttonStyle(.plain).opacity(hoveredExpenseId == expense.id ? 1 : 0)
            }.frame(width: 80)
    ...
    private func editingRow(_ expense: Expense) -> some View {
        HStack(spacing: 0) {
            ...
            Picker("", selection: $editCategory) { Text("None").tag(nil as ExpenseCategory?); ForEach(categories, id: \.id) { cat in Label(cat.name, systemImage: cat.icon).tag(cat as ExpenseCategory?) } }.labelsHidden().frame(width: 160)
    ...
    private var emptyState: some View {
        VStack(spacing: 16) {
            Spacer(); Image(systemName: "tray").font(.system(size: 48)).foregroundColor(AppTheme.textMuted)
