...
struct CategoryManagementView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack {
                ...
                Spacer()
                Button {
                    formName = ""; formIcon = "tag.fill"; formColor = "#7C3AED"
                    showAddForm = true
                } label: {
        ...
        .sheet(item: $editingCategory) { cat in
            categoryForm(editing: cat)
                .onAppear { formName = cat.name; formIcon = cat.icon; formColor = cat.colorHex }
        }
    }
    ...
    private func categoryCard(_ cat: ExpenseCategory) -> some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack(spacing: 12) {
                ZStack {
                    RoundedRectangle(cornerRadius: 12).fill(cat.color.opacity(0.15)).frame(width: 44, height: 44)
                    Image(systemName: cat.icon).font(.system(size: 20)).foregroundColor(cat.color)
                }
        ...
        .padding(18)
        .background(
            RoundedRectangle(cornerRadius: 16).fill(AppTheme.surface.opacity(0.7))
                .overlay(RoundedRectangle(cornerRadius: 16).stroke(hoveredCategoryId == cat.id ? cat.color.opacity(0.4) : AppTheme.border.opacity(0.3), lineWidth: 1))
        )
    ...
    private func categoryForm(editing: ExpenseCategory?) -> some View {
        VStack(spacing: 0) {
            VStack(spacing: 10) {
                ZStack {
                    RoundedRectangle(cornerRadius: 16).fill(Color(hex: formColor).opacity(0.15)).frame(width: 60, height: 60)
                    Image(systemName: formIcon).font(.system(size: 28)).foregroundColor(Color(hex: formColor))
                }
                Text(editing != nil ? "Edit Category" : "New Category")
                    .font(.system(size: 20, weight: .bold, design: .rounded)).foregroundColor(AppTheme.textPrimary)
            }
            .frame(maxWidth: .infinity).padding(.vertical, 24).background(AppTheme.subtleGradient)
            
            VStack(spacing: 20) {
                VStack(alignment: .leading, spacing: 8) {
                    Text("Name").font(.system(size: 12, weight: .semibold)).foregroundColor(AppTheme.textSecondary)
                    TextField("Category name", text: $formName).textFieldStyle(.plain).font(.system(size: 14))
                        .foregroundColor(AppTheme.textPrimary).padding(12)
                    ...
                    LazyVGrid(columns: Array(repeating: GridItem(.fixed(40), spacing: 8), count: 8), spacing: 8) {
                        ForEach(icons, id: \.self) { ic in
                            Button { formIcon = ic } label: {
                                Image(systemName: ic).font(.system(size: 16))
                                    .foregroundColor(formIcon == ic ? Color(hex: formColor) : AppTheme.textSecondary)
                                    .frame(width: 36, height: 36)
                                    .background(RoundedRectangle(cornerRadius: 8).fill(formIcon == ic ? Color(hex: formColor).opacity(0.15) : AppTheme.surfaceElevated)
                                        .overlay(RoundedRectangle(cornerRadius: 8).stroke(formIcon == ic ? Color(hex: formColor).opacity(0.5) : Color.clear, lineWidth: 1.5)))
                            }.buttonStyle(.plain)
                        }
                    }
                }
                        ...
                        ForEach(colors, id: \.self) { c in
                            Button { formColor = c } label: {
                                Circle().fill(Color(hex: c)).frame(width: 28, height: 28)
                                    .overlay(Circle().stroke(Color.white, lineWidth: formColor == c ? 2.5 : 0))
                                    .scaleEffect(formColor == c ? 1.15 : 1.0)
                                    .animation(.spring(response: 0.3), value: formColor == c)
                            }.buttonStyle(.plain)
                        }
                    }
                }
            ...
            HStack {
                Button("Cancel") { showAddForm = false; editingCategory = nil }
                    .font(.system(size: 13, weight: .medium)).foregroundColor(AppTheme.textSecondary)
                    .padding(.horizontal, 20).padding(.vertical, 10)
                    .background(RoundedRectangle(cornerRadius: 10).fill(AppTheme.surfaceElevated)).buttonStyle(.plain)
                Spacer()
                Button {
                    if let cat = editing { cat.name = formName; cat.icon = formIcon; cat.colorHex = formColor; try? modelContext.save(); editingCategory = nil }
                    else { modelContext.insert(ExpenseCategory(name: formName, icon: formIcon, colorHex: formColor)); try? modelContext.save(); showAddForm = false }
                } label: { Text(editing != nil ? "Save" : "Add").gradientButton() }
                .buttonStyle(.plain).disabled(formName.isEmpty)
            }.padding(20)
        }
